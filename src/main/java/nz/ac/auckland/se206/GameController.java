...
public class GameController {
	...
	@FXML
	private Label preGameCategoryLabel;
	@FXML
	private AnchorPane preGamePane;
	@FXML
	private VBox postGame;
	@FXML
	private Label postGameOutcomeLabel;
	@FXML
	private VBox game;
	@FXML
	private Label timerLabel;
	@FXML
	private Label predictionsList;
				...
				while (!gameWon && timerThread.isAlive()) {
					...
					if (deltaTime >= 1000) {
						if (doPredict) {
							...
							FutureTask<BufferedImage> snapshotTask = new FutureTask<BufferedImage>(
									(Callable) () -> getCurrentSnapshot());
							Platform.runLater(snapshotTask);
							java.util.List<Classifications.Classification> predictions = model
									.getPredictions(snapshotTask.get(), 10);
	...
	private void setPostGame(int timeTaken) {
		...
		Task<Void> saveUserDataTask = new Task<Void>() {
			@Override
			protected Void call() throws Exception {
				...
				try {
					...
					FileWriter fileWriter = new FileWriter(
							"src/main/resources/UserProfiles/" + user.getName() + ".json");
